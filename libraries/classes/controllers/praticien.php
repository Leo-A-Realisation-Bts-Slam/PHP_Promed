...
<!-- Ce code est un script PHP qui définit une classe "Praticien" qui hérite d'une classe "Controller".
La classe "Praticien" contient plusieurs méthodes qui effectuent des actions spécifiques lorsque ces méthodes
sont appelées. Ces actions incluent afficher une authentification pour les praticiens,
rediriger vers l'espace personnel d'un praticien, afficher l'accueil du site, afficher un formulaire d'inscription pour les praticiens et 
enregistrer les informations d'inscription d'un praticien dans la base de données. Il utilise les classes de modèles "Rendez_vous" et "Praticien"
pour récupérer des informations de la base de données. Il utilise aussi la classe "Renderer" pour afficher les vues.
Il utilise les données postées pour récupérer les données d'inscription d'un praticien, et utilise les fonctions de filtrage de données pour sécuriser les données d'entrée. 
Il utilise aussi la fonction password_hash pour hasher le mot de passe d'un praticien. -->
